...
namespace Dropbox
{
    ...
    [Guid("9ea79d7b-c77f-4e6a-a210-8cd323ab9526")]

    public class DropboxImp : EurekaSim.Net.Addin,
        ...
        IFileSettingsTreeViewEvents
    {
        ...
        public void Initialize(int lSessionID, MainApplication pApp, object bFirstTime)
        {
            ...
            Stream rgbxml = thisAssembly.GetManifestResourceStream("Dropbox.AddinRibbon.xml");

            Stream rgbBmp = thisAssembly.GetManifestResourceStream("Dropbox.toolbar1.bmp");
        ...
        public void InvokeUploadDlg()
        {
            ...
            {
                DialogResult result = MessageBox.Show("You are not logged in to Dropbox. Continue to login?", "EurekaSim Dropbox Backup", MessageBoxButtons.YesNo);
                if (result == DialogResult.Yes)
                {
                    DbSettings settingsDlg = new DbSettings();
		...
		public async void OnAfterSaveDocument(string DocumentPath)
        {
            m_saveCheck = 2;
            ...
            if (autoSave && token != "")
            {
                ...
                var client = new DropboxClient(token, config);
                FileMetadata response = null;
                try
                {
                    using (var stream = new MemoryStream(content))
                    {
                        response = await client.Files.UploadAsync("/" + dfltUploadLoc + "/" + fileNme, WriteMode.Overwrite.Instance, body: stream);
                        MessageBox.Show("File backed up succssfully \n" + response.Id, "EurekaSim Dropbox Addin");
                    }
                }
                catch (Exception ex)
                {
