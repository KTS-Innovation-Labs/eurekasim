...
namespace ESimBoxCloud
{
	...
	public class AddinSimulationManager
    {
		...
		public string m_strExperimentType;
		public bool m_bSimulationActive;
		public ESimBoxCloudImp m_pAddin;
        ...
        #region Constructor
        public AddinSimulationManager(ESimBoxCloudImp pAddin)
		{
			m_pAddin = pAddin;
			m_pObjectDemoExperiment = new ObjectDemoExperiment(this);
			m_strExperimentType = Constants.CS_SAMPLE_EXPERIMENT_TYPE_GROUP_1;
        ...
        {
			if (!IsAddinExperimentSelected() || string.IsNullOrEmpty(m_strExperimentType))
			{
			...
			if (m_strExperimentType == Constants.CS_SAMPLE_EXPERIMENT_TYPE_GROUP_1)
            {
		...
		public void OnStatusChange(int statusCode, string statusDesc, string additionalParam)
		{
			...
			else if (statusCode == 10001 && m_pAddin.m_lSessionID == Convert.ToInt32(additionalParam)) //Experiment Changed 
			{
				...
				m_pAddin.SetRibbonControlText("Cs.Sample.Experimental.Setup.Select.Experiment", m_strExperimentType);
		...
		public void LoadExperiments(string ExperimentType)
		{
			...
			if (ExperimentType == Constants.CS_SAMPLE_EXPERIMENT_TYPE_GROUP_1)
			{
				m_pObjectDemoExperiment.LoadAllExperiments();
			}
		...
		public string Serialize()
		{
			...
			try
			{
				...
				info.ExperimentType = m_strExperimentType;
		...
		public bool DeSerialize(string strXML)
		{
			...
			try
			{
				...
				m_strExperimentType = info.ExperimentType;
