...
BOOL CMySimulationAddin::LoadLongResource(CString &str, UINT nID)
{
	try
	{
		HRSRC               hRes;
...
STDMETHODIMP CMySimulationAddin::InvokePreferenceSettings()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	AfxMessageBox(_T("CMySimulationAddin::InvokePreferenceSettings()"));

	return S_OK;
}


STDMETHODIMP CMySimulationAddin::InvokeDefaultSettings()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	AfxMessageBox(_T("CMySimulationAddin::InvokeDefaultSettings(Hai)"));

	return S_OK;
}


STDMETHODIMP CMySimulationAddin::InvokeOnExperimentChanged()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
		BSTR ControlText;
		RibbonToolbar->GetControlText(_T("Cpp.Sample.Experimental.Setup.Select.Experiment"), &ControlText);
...
void CMySimulationAddin::SetRibbonControlText(CString ID, CString Text)
{
	CComPtr<IRibbonToolbar> RibbonToolbar;
	HRESULT HR = RibbonToolbar.CoCreateInstance(CLSID_RibbonToolbar);
	if (SUCCEEDED(HR))
...
void CMySimulationAddin::LoadAddinName(CString strXML)
{
	try
	{
		USES_CONVERSION;
