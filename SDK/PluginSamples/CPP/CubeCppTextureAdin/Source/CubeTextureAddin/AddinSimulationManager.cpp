...
CAddinSimulationManager::CAddinSimulationManager(CCubeTextureAddinSimulation* pAddin)
{
	m_pAddin = pAddin;
...
{
	if (m_pObjectDemoExperiment->m_ObjectPattern.m_strObjectType == OBJECT_TYPE_BANNER_CUBE)
	{
		m_pObjectDemoExperiment->DrawBannerCube();
	}
	else if (m_pObjectDemoExperiment->m_ObjectPattern.m_strObjectType == OBJECT_TYPE_DICE)
	{
		m_pObjectDemoExperiment->DrawDice();
	}
}
