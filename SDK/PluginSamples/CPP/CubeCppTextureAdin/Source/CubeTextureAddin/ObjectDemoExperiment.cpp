...
void CObjectDemoExperiment::DrawBannerCube()
{
	...
	if (!m_TextureID)
	{
		hBanner = (HBITMAP)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_BITMAP1), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
		if (hBanner)
		{
			//Render Bitmap
			m_TextureID = 0;
			...
			//OpenGLView->glTexImage2D(GL_TEXTURE_2D, 0, GL_BGR_EXT, BM.bmWidth, BM.bmHeight, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, BitmapData);
...
void CObjectDemoExperiment::DrawDice()
{
	...
	if (!m_frontId)
	{
		m_frontId = LoadTexture(IDB_BITMAP2);
	}
	if (!m_backId)
	{
		m_backId = LoadTexture(IDB_BITMAP3);
	}
	if (!m_topId)
	{
		m_topId = LoadTexture(IDB_BITMAP4);
	}
	if (!m_bottomId)
	{
		m_bottomId = LoadTexture(IDB_BITMAP5);
	}
	if (!m_rightId)
	{
		m_rightId = LoadTexture(IDB_BITMAP6);
	}
	if (!m_leftId)
	{
		m_leftId = LoadTexture(IDB_BITMAP7);
	}
	DrawFrontFaceCube(m_frontId);
	DrawBackFaceCube(m_backId);
	DrawTopFaceCube(m_topId);
	DrawBottomFaceCube(m_bottomId);
	DrawRightFaceCube(m_rightId);
	DrawLeftFaceCube(m_leftId);

}

unsigned int CObjectDemoExperiment::LoadTexture(UINT resourceId)
{
	ULONG textureID = 0;
	//Draw using Native IOpenGLView Interface
	CComPtr<IOpenGLView> OpenGLView;
