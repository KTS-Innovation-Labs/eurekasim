...
STDMETHODIMP CMyCppAddinSimulation::InvokeDefaultSettings()
{
	...
	AfxMessageBox(_T("CMyCppAddinSimulation::InvokeDefaultSettings(Hai)"));

	return S_OK;
}


STDMETHODIMP CMyCppAddinSimulation::InvokeOnExperimentChanged()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CMyCppAddinSimulation::SetRibbonControlText(CString ID, CString Text)
{
	CComPtr<IRibbonToolbar> RibbonToolbar;
	HRESULT HR = RibbonToolbar.CoCreateInstance(CLSID_RibbonToolbar);
	if (SUCCEEDED(HR))
...
void CMyCppAddinSimulation::LoadAddinName(CString strXML)
{
	try
	{
		USES_CONVERSION;
