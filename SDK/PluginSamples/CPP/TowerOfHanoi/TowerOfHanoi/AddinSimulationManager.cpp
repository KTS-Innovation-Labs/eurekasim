...
CAddinSimulationManager::CAddinSimulationManager(CTowerOfHanoiSimulation* pAddin)
{
	m_pAddin = pAddin;
...
HRESULT CAddinSimulationManager::OnStartSimulation(BSTR ExperimentName)
{
	...
	m_pObjectDemoExperiment->StartSimulation(m_strExperimentGroup.AllocSysString(),
		m_strExperimentName.AllocSysString());
	return S_OK;
}
...
HRESULT CAddinSimulationManager::OnStopSimulation(BSTR ExperimentName)
{
	...
	SetStatusBarMessage(_T("Simulation Stopped."), TRUE);
	return S_OK;
}
...
{
	// FIX: The following code is re-enabled to prevent OpenGL errors. When the user
	// clicks and drags the mouse in the 3D view, the main UI thread tries to render
	// the view transformations. If the simulation thread is also rendering, it
	// causes a conflict. Pausing the simulation on mouse down avoids this issue.
}
...
{
	CStringW strCtrlID(CtrlID);

	if (strCtrlID == L"Start")
	{
		*pStatus = !m_bSimulationActive;
	}
	else if (strCtrlID == L"Stop")
	{
		*pStatus = m_bSimulationActive;
	}
	else
