...
namespace EmWaveCs
{
    ...
    static class Constants
    {
        ...
        public const string OBJECT_TYPES = "Cube,Ball,Pyramid,Aeroplane,Clock,EmWave";
        ...
        public const string OBJECT_TYPE_EMWAVE = "EmWave";

        public const string OBJECT_PATTERN_TYPES = "Rotate,Random Movement,Propagate";
        public const string OBJECT_PATTERN_TYPE_ROTATE = "Rotate";
        public const string OBJECT_PATTERN_TYPE_RANDOM = "Random Movement";
        public const string OBJECT_PATTERN_TYPE_PROPAGATE = "Propagate";
        ...
        public const string CS_SAMPLE_MAIN_EXPERIMENT_NAME = "EmWaveCs Experiment Simulation Demo";
    ...
    public class ObjectDemoExperiment
    {
        ...
        public void DrawObject(string ExperimentName)
        {
            ...
            else if (m_ObjectPattern.m_strObjectType == Constants.OBJECT_TYPE_EMWAVE)
            {
                DrawEmWave();
            }
        }
        public void DrawEmWave()
        {
            ...
            const float length = 8.0f;       // total length of wave
            const int num_cycles = 2;        // number of wavelengths
            const int segments = 200;        // smoothness
            float lambda = length / num_cycles;   // wavelength
            float k = (2.0f * (float)Math.PI) / lambda;   // wave number

            // --- Time-dependent phase ---
            if (_t == 0) _t = 0.0f;   // init on first call
            _t += 0.3f;               // controls animation speed
            float omega = 2.0f * (float)Math.PI * 0.7f;  // angular frequency
            ...
            for (int i = 0; i <= segments; i++)
            {
                ...
                float y_val = amplitude * (float)Math.Sin(k * z - omega * _t);
            ...
            {
                float z = z_start + i * z_step;
                float x_val = amplitude * (float)Math.Sin(k * z - omega * _t);
            ...
            openGLView.glBegin(Constants.GL_POINTS);
        ...
        // add this field at class level (to persist time)
        ...
        public void StartObjectSimulation()
        {
            ...
            while (m_pManager.m_bSimulationActive)
            {
                ...
                else if (m_ObjectPattern.m_strSimulationPattern == Constants.OBJECT_PATTERN_TYPE_PROPAGATE)
                {
                    // EM wave propagation
                    DrawEmWave();
