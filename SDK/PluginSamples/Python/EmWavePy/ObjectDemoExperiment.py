...
class ObjectDemoExperiment():
    ...
    def __init__(self,objManager):
        ...
        self._t = 0.0  # Time variable for EM wave animation
    ...
    def DrawObject(self,ExperimentName):
        try:
            if self.m_ObjectPattern.m_strObjectType == OBJECT_TYPE_CUBE:
                ...
                self.DrawClock()
            elif self.m_ObjectPattern.m_strObjectType == OBJECT_TYPE_EMWAVE:
                self.DrawEmWave()
        except Exception as ex:
            win32ui.MessageBox(ex)
    ...
    def DrawEmWave(self):
        try:
            ...
            # Background = black
            applicationView.SetBkgColor(0.0, 0.0, 0.0, 1.0)
            applicationView.StartNewDisplayList()

            # Create OpenGL view for low-level drawing
            openGLView = OpenGLView()

            # --- Wave Parameters ---
            amplitude = 1.2
            length = 8.0       # total length of wave
            num_cycles = 2     # number of wavelengths
            segments = 200     # smoothness
            wavelength = length / num_cycles   # wavelength
            k = (2.0 * math.pi) / wavelength   # wave number

            # --- Time-dependent phase ---
            if self._t == 0: 
                self._t = 0.0   # init on first call
            self._t += 0.3      # controls animation speed
            omega = 2.0 * math.pi * 0.7  # angular frequency

            z_start = -length / 2.0
            z_step = length / segments

            # --- 1. Electric Field (Red, Y-axis oscillation) ---
            openGLView.glColor3f(1.0, 0.2, 0.2)
            openGLView.glBegin(GL_LINES)
            for i in range(segments + 1):
                z = z_start + i * z_step
                y_val = amplitude * math.sin(k * z - omega * self._t)
                openGLView.glVertex3f(0.0, -y_val, z)
                openGLView.glVertex3f(0.0, y_val, z)
            openGLView.glEnd()
            ...
            for i in range(segments + 1):
                ...
                openGLView.glVertex3f(0.0, -y_val, z)
            openGLView.glEnd()
            openGLView.glLineWidth(1.0)

            # --- 2. Magnetic Field (Blue, X-axis oscillation) ---
            openGLView.glColor3f(0.2, 0.5, 1.0)
            openGLView.glBegin(GL_TRIANGLE_STRIP)
            for i in range(segments + 1):
                z = z_start + i * z_step
                x_val = amplitude * math.sin(k * z - omega * self._t)
                openGLView.glVertex3f(-x_val, 0.0, z)
                openGLView.glVertex3f(x_val, 0.0, z)
            openGLView.glEnd()

            # --- 3. White dot at origin (charge source) ---
            openGLView.glPointSize(8.0)
            ...
            while self.m_objManager.m_bSimulationActive:
                ...
                if self.m_ObjectPattern.m_strSimulationPattern == OBJECT_PATTERN_TYPE_ROTATE:
                    ...
                    if i == 0:
                        ...
                        z = 1.0
                    i = rnd.randint(0, 2)
                elif self.m_ObjectPattern.m_strSimulationPattern == OBJECT_PATTERN_TYPE_PROPAGATE:
                    # EM wave propagation
                    self.DrawEmWave()
                    # Do not rotate the object in propagate mode
                    Angle = 0.0
                    x = 0.0
                    y = 0.0
                    z = 0.0
                    ...
                    continue # skip the rotation logic below
                
                if not self.m_objManager.m_b3DMode:
                ...
                if Angle > 360:
                    Angle = 0
                time.sleep(self.m_ObjectPattern.m_lSimulationInterval/1000) #Sleep for specified Milli seconds
    ...
    def DisplayObjectDemoGraph(self):
        try:
            ...
            arraySize = [iArraySize, 2]
            # Create 2D arrays for the data
            sabX = [[0.0 for _ in range(2)] for _ in range(iArraySize)]
            sabY = [[0.0 for _ in range(2)] for _ in range(iArraySize)]
            sabZ = [[0.0 for _ in range(2)] for _ in range(iArraySize)]

            for i in range(iArraySize):
                try:
                    info = self.m_PlotInfoArray[i]
                    # Set X values (Angle in first column, x in second column)
                    sabX[i][0] = info.m_Angle
                    sabX[i][1] = info.m_x
                    
                    # Set Y values (Angle in first column, y in second column)
                    sabY[i][0] = info.m_Angle
                    sabY[i][1] = info.m_y
                    
                    # Set Z values (Angle in first column, z in second column)
                    sabZ[i][0] = info.m_Angle
                    sabZ[i][1] = info.m_z
                except Exception as e:
                    win32ui.MessageBox(str(e))

            # Convert to VARIANT arrays
