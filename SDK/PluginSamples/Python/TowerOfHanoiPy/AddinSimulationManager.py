...
# Methods from IExperimentEvents
    ...
    def OnInitializeSimulationGraph(self, ExperimentName=defaultNamedNotOptArg):
        if self.m_strExperimentType == PY_SAMPLE_EXPERIMENT_TYPE_GROUP_1:
    ...
    def OnStartSimulation(self, ExperimentName=defaultNamedNotOptArg):
        if self.IsAddinExperimentSelected()==False or self.m_strExperimentType=="":
            strMessage= "Failed to Start "+ExperimentName+". Please select Experimental Parameters\
                 before starting the Simulation Experiment.."
    ...
    def OnStatusChange(self, StatusCode=defaultNamedNotOptArg, StatusDesc=defaultNamedNotOptArg, AdditionalParam=defaultNamedNotOptArg):
        if  StatusCode == 10000: #No Experiment Selected
            ...
            self.m_objAddin.SetRibbonControlText("Py.Sample.Experimental.Setup.Select.Experiment", self.m_strExperimentType)
            self.m_strSelectedExperiment = StatusDesc
...
# Methods from IRibbonToolbarEvents
    def GetControlStatus(self, CtrlID=defaultNamedNotOptArg, pStatus=defaultNamedNotOptArg):
        if CtrlID == "Start":
             pStatus = not self.m_bSimulationActive
        elif CtrlID == "Stop":
            pStatus = self.m_bSimulationActive
        else:
            pStatus = True
...
#  Other Dependent Methods
    def LoadDefaultSelection(self):
        ...
        if self.m_objAddin.m_lSessionID == SessionID:
           self.LoadExperiments(self.m_strExperimentType)
    ...
    def LoadExperiments(self,ExperimentType):
        if self.IsAddinExperimentSelected() == False:
            return
        if ExperimentType == PY_SAMPLE_EXPERIMENT_TYPE_GROUP_1:
            self.m_ObjectDemoExperiment.LoadAllExperiments()
        self.m_strExperimentType = ExperimentType
    ...
    def Serialize(self):
        try:
            ...
            info.ExperimentType = self.m_strExperimentType
    ...
    def DeSerialize(self,strXMLData):
        try:
            ...
            self.m_strExperimentType = info.ExperimentType
