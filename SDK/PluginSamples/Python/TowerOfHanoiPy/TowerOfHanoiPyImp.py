...
#   TowerOfHanoiPyImp.py --unregister
#
# To debug, execute:
#   TowerOfHanoiPyImp.py --debug
#
...
class TowerOfHanoiPyImp():
    ...
    _reg_clsid_ = "{a6023713-c7b7-4b7c-b36c-ad6a6d1a0180}"
    _reg_progid_ = "TowerOfHanoiPyImp"
    ...
    def Initialize(self, lSessionID, pApp, bFirstTime):
        try:
            ...
            # Set the plugin name in the manager
            self.m_objAddinSimulationManager.SetPluginName("TowerOfHanoiPy Addin")
...
# Methods For XML Ribbon Toolbar
    ...
    def InvokeDefaultSettings(self):
        try:
            if self.m_objAddinSimulationManager and self.m_objAddinSimulationManager.m_ObjectDemoExperiment:
                # Set the object type to Tower of Hanoi to ensure the correct properties are displayed
                self.m_objAddinSimulationManager.m_ObjectDemoExperiment.m_ObjectPattern.m_strObjectType = OBJECT_TYPE_TOWEROFHANOI
                # Show the corresponding properties in the property window
                self.m_objAddinSimulationManager.m_ObjectDemoExperiment.ShowObjectProperties()
        except Exception as ex:
            win32ui.MessageBox(str(ex))

    def InvokeOnExperimentChanged(self):
        # This function is no longer called by the ribbon but is kept for completeness
...
# Methods from IApplicationDocumentEvents
    ...
    def OnAfterSaveDocument(self, DocumentPath=defaultNamedNotOptArg):
       # In the C++ example, this method is empty.
       pass
    ...
    def OnNewDocument(self, DocumentName=defaultNamedNotOptArg):
         self.m_objAddinSimulationManager.OnNewDocument(DocumentName)
...
if __name__ == '__main__':
    ...
    win32com.server.register.UseCommandLine(TowerOfHanoiPyImp)
    if "--unregister" in sys.argv:
        UnregisterAddin(TowerOfHanoiPyImp)
    else:
        RegisterAddin(TowerOfHanoiPyImp)
